...
- **P77-EX8: 证明：当放置$(k+1)_{th}$皇后时，若有多个位置是开放的，则算法QueensLV选中其中任一位置的概率相等。**
  ...
  假设循环到第$m(m\in [1,n-1])$个位置时选中最后一个位置，剩余未访问的位置的在将来访问时皆未被选中，第$m$轮选中该位置的概率为$\frac{1}{m}$，根据乘法原则，剩余位置未被选择的概率为$$\prod_{i=m+1}^{n}\frac{i-1}{i}=\frac{m}{n}$$所以最终选择第$m$个位置的概率为$\frac{1}{m}\times \frac{m}{n}=\frac{1}{n}$。
  当选中最后一个位置时，其概率为$\frac{1}{n}$。
